  ...
  <body>
    ...
    <div id="user" class="subMenu">
      <div id="user" class="form">
        <label for="username">Usuario<input type="text" id="username" class="input" required/></label>
        <label for="password">Contraseña<input type="password" id="password" class="input" required/></label>
        <button class="button" @click="login()">Iniciar Sesión</button>
        <button class="button" @click="register()">Regístrate</button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    login () {
      ...
      var f = this.axios.post('user/login', logIn)
        .then((result) => {
          console.log(result.data)
          saludo.innerHTML = username.value
    ...
    register () {
      ...
      var f = this.axios.post('/user/register', logIn)
        .then((result) => {
          console.log(result.data)
          saludo.innerHTML = username.value
          document.getElementById('user').style.display = 'none'
